...
class PostedComment {
    ...
    favorites(toFavorites) {
        ...
        toFavorites.innerHTML = `${this.isFavorite
            ? `<img src="svg/addtofavorites.svg" alt=""><span>В избранном</span>`
            : `<img src="svg/white_heart.svg" alt=""><span>В избранное</span>`}`;
    ...
    getHTMLElement(name) {
        ...
        if (this.isReply) {
            ...
            replyTo.innerHTML = `<img src="svg/reply.svg" alt=""><span>${name}</span>`;
            wrapperNameReply.appendChild(replyTo);
        }
        if (!this.isReply) {
            const replyButton = document.createElement('button');
            replyButton.classList.add('reply');
            replyButton.innerHTML = `<img src="svg/reply.svg" alt=""><span>Ответить</span>`;
